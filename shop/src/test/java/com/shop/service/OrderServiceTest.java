...
public class OrderServiceTest {
    ...
    // 테스트를 위해 주문할 상품과 회원 정보를 저장하는 메소드 생성
    ...
    public void order(){
        ...
        Order order = orderRepository.findById(orderId).orElseThrow(EntityNotFoundException::new);  // 주문 번호를 이용해 저장된 주문 정보를 조회
    ...
    public void cancelOrder(){
        Item item = saveItem(); // 테스트를 위해 상품과 회원 데이터를 생성
        Member member = saveMember();

        OrderDto orderDto = new OrderDto(); // 테스트를 위해 주문 데이터 생성
        orderDto.setCount(10);
        orderDto.setItemId(item.getId());
        Long orderId = orderService.order(orderDto, member.getEmail());

        Order order = orderRepository.findById(orderId).orElseThrow(EntityNotFoundException::new);  // 생성한 주문 엔티티 조회
        orderService.cancelOrder(orderId);  // 해당 주문을 취소함
