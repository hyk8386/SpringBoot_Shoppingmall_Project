...
public class ItemService {
    ...
    @Transactional(readOnly = true)    // 상품 데이터를 읽어오는 트랜잭션을 읽기 전용으로 설정 > 이럴 경우 JPA가 더티체킹을 수행하지 않아 성능을 향상 시킬 수 있음
    public ItemFormDto getItemDtl(Long itemId){
        ...
        // 상품의 아이디를 통해 상품 엔티티 조회, 존재하지 않을 때는 EntityNotFoundException 발생
    ...
    public Long updateItem(ItemFormDto itemFormDto, List<MultipartFile> itemImgFileList) throws Exception {
        ...
        // 상품 등록화면으로부터 전달 받은 상품 아이디를 이용해 상품 엔티티를 조회
        item.updateItem(itemFormDto);
        // 상품 등록화면으로부터 전달 받은 itemFormDto를 통해 상품 엔티티를 업데이트
